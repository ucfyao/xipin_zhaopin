...
class model{
	...
	function insert_recycle($tablename,$where){
		...
		if(!isset($_GET['isdel']) || $_GET['isdel']!="all"){
			...
			$query = $this->db->query("SELECT * FROM `" . $this->def . $tablename . "` WHERE $where");
			$row_del = array();
