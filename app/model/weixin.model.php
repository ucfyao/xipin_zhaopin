...
class weixin_model extends model{
	...
	function searchJob($keyword)
	{
		...
		if($keyword)
		{
			...
			if(is_array($keywords))
			{
				...
				$searchWhere = "`state`='1' AND `sdate`<='".time()."' AND `status`<>'1' AND `r_status`<>'2'";
				if(!empty($searchJob))
				{
	...
	function recJob()
	{
		...
		$JobList = $this->DB_select_all("company_job","`sdate`<='".time()."' AND `status`<>1 AND `r_status`<>2 AND `rec_time`>'".time()."' order by `lastupdate` desc limit 5","`id`,`name`,`com_name`,`lastupdate`");
