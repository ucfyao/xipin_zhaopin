...
function smarty_function_jobsimple($paramer,&$smarty){
	...
	if($paramer[type]=="bid")
	{
		$query=$db->query("SELECT name,com_name,id,uid FROM $db_config[def]company_job WHERE `xuanshang`!='' AND `state`='1' AND `status`!='1' AND `sdate`<='$time' $sitesql  order by `xuanshang` desc $limit");//竞价招聘职位
		...
		$query=$db->query("SELECT * FROM $db_config[def]company_job WHERE AND `sdate`<='$time' AND `state`='1' AND `status`!='1' $sitesql order by 'lastupdate' DESC $limit");
		...
		$query=$db->query("SELECT * FROM $db_config[def]company_job WHERE `sdate`<='$time' AND `state`='1' AND `status`!='1' $sitesql order by 'hits' DESC limit 3");
	}
