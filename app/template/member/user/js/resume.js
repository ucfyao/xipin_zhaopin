	...
	$.post("index.php?c=expect&act=saveexpect", {name:name, hy:hy, job_classid: job_classid, city_classid: city_classid, minsalary: minsalary, maxsalary: maxsalary, jobstatus: jobstatus, type: type, report: report, eid: eid, submit: "1", dom_sort: getDomSort() }, function (data) {
		...
		if(data==0){
			...
			if(eid==""){
				...
				var html='<li>期望岗位：'+data.name+'</li><li>工作职能：'+data.job_classname+'</li><li>期望工作地点：'+data.city_classname+'</li><li>期望从事行业：'+data.hy+'</li><li>期望薪资：'+salary+'</li><li>到岗时间：'+data.report+'</li><li>期望工作性质：'+data.type+'</li><li>求职状态：'+data.jobstatus+'</li>';
				$("#expect").html(html);
	...
	if(message != null){
		...
		$.post(weburl+"/member/index.php?c=expect&act=showresume",{eid:eid,resume:resume},function(data){
			...
			if(data!=0){
				...
				$(".used"+resume).val('1');
				$("#"+resume).html(data);
				layer.msg('操作成功！', 2,9,function(){
					window.location.reload();
					ScrollTo(resume+"_upbox");
				})
			}else{ 
				layer.msg('操作失败！', 2,8);
			}
