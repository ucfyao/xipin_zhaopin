...
class index_controller extends common{
	...
	function loginsave_action(){
		...
		if($this->config['sy_uc_type']=="uc_center"  && !$_POST['act_login']){
			...
			$user = $Member->GetMemberOne($where,array("field"=>"`pw_repeat`,`pwuid`,`uid`,`username`,`salt`,`email`,`moblie`,`password`,`usertype`,`status`,`email_status`,`did`,`login_date`"));
			
			if(!$user && CheckMoblie($username)){
				$user = $UserinfoM->GetMemberOne(array("`moblie`='".$username."'"),array('field'=>"username,usertype,password,uid,usertype,salt,status,did,login_date"));
			...
			if(!$user&&CheckRegEmail($username)){
				$user = $UserinfoM->GetMemberOne(array("`email`='".$username."'"),array('field'=>"username,usertype,password,uid,usertype,salt,status,did,login_date"));
