	...
	function add_action(){
		...
		if($_POST['submit']){
			...
			if($id==''){
				...
				$arr = $TinyM->GetTinyresumeOne(array('id'=>$id,'password'=>$password),array('field'=>'id'));
				
				if($arr['id']){
					$nid=$this->obj->update_once("resume_tiny",$_POST,"`id`='".$id."'");
 					$nid?$data['msg']=$msg:$data['msg']='操作失败！';
					$data['url']=Url('wap',array('c'=>'tiny'));
				}else{
					$data['msg']='密码错误！';
					$data['url']=Url('wap',array('c'=>'tiny','a'=>'show','id'=>$id));
				}
			}
            $data=$data;
