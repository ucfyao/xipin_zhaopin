...
class ask_controller extends common{
			...
			if($authcode!=$_SESSION['authcode'] || empty($_SESSION['authcode'])){ 
				...
				echo '0';die;		
 			    ...
 			    if($this->config['integral_answer_type']==1){
			        ...
			        $statis=$UserInfoM->GetUserstatisOne(array("uid"=>$this->uid),array("field"=>"`integral`","usertype"=>$this->usertype));
	...
	function myquestion_action(){
		...
		if($uid){
		    $member = $this->obj->DB_select_once("member","`uid`='".$uid."'","`uid`,`usertype`");
		    $usertype=$member['usertype'];
		    $MemberM=$this->MODEL("userinfo");
		    $minfo=$MemberM->GetUserinfoOne(array("uid"=>$uid),array("usertype"=>$usertype));
		    if($usertype==2){
		        if($minfo['logo'] && file_exists(str_replace('./data', APP_PATH.'/data', $minfo['logo']))){
		            $row['pic']=str_replace('./data', $this->config['sy_weburl'].'/data', $minfo['logo']);
		        }
		    }elseif($usertype==1){
		        if($minfo['photo'] && file_exists(str_replace('./data', APP_PATH.'/data', $minfo['photo']))){
		            $row['pic']=str_replace('./data', $this->config['sy_weburl'].'/data', $minfo['photo']);
		        }
		    }
		...
		$this->yunset("row",$row);
		$info=$M->GetQuestionOne(array('uid'=>$uid),array('field'=>'nickname'));
		if($info['nickname']){
		...
		$q_num = $this->obj->DB_select_num("question","`uid`='".$uid."'");
		$this->yunset("q_num",$q_num?$q_num:0);
		
		$qw_num = $this->obj->DB_select_num("question","`uid`='".$uid."'and `state`='1'");
		$this->yunset("qw_num",$qw_num);
			
		$a_num = $this->obj->DB_select_num("answer","`uid`='".$uid."'");
		...
		if(count($ids)){
 			$g_num=$this->obj->DB_select_num("question","`id` in (".pylode(',',$ids).")");
		}
