...
class login_controller extends common{
				...
				if($this->config['sy_uc_type']=="uc_center"){
						...
						if(!empty($user)){
							...
							if($this->config['user_status']=="1"){
								if($user['usertype']=='1'){
									...
									$info=$Resume->SelectResume(array("uid"=>$user['uid']),"`name`,`email_status`,`birthday`");
					...
					if ($this->config['sy_msg_isopen'] && $this->config['sy_msg_login'] && $_POST['act_login']) {
						...
						if(!$userinfo&&CheckMoblie($username)){
							$userinfo = $UserinfoM->GetMemberOne(array("`moblie`='".$username."'"),array('field'=>"username,usertype,password,uid,usertype,salt,status,did,login_date"));
							$binding=$UserinfoM->GetUserinfoOne(array("`uid`=".$userinfo['uid']." and `moblie_status`=1"),array('usertype'=>$userinfo['usertype'],'field'=>"uid"));
							if(!$binding){
								$data['msg']='手机号未绑定，不能登录！';
								$this->layer_msg($data['msg'],9,0,'',2);
							}
						}
						if(!$userinfo&&CheckRegEmail($username)){
							$userinfo = $UserinfoM->GetMemberOne(array("`email`='".$username."'"),array('field'=>"username,usertype,password,uid,usertype,salt,status,did,login_date"));
							$binding=$UserinfoM->GetUserinfoOne(array("`uid`=".$userinfo['uid']." and `email_status`=1"),array('usertype'=>$userinfo['usertype'],'field'=>"uid"));
							if(!$binding){
								$data['msg']='邮箱未绑定，不能登录！';
								$this->layer_msg($data['msg'],9,0,'',2);
							}
					...
					if($res){
						...
						if($userinfo['usertype']=='1'){
							$Resume=$this->MODEL("resume");
							$info=$Resume->SelectResume(array("uid"=>$userinfo['uid']),"`email_status`");
	...
	function sendmsg_action(){
		if(!$this->config['sy_msg_isopen'] || !$this->config['sy_msg_login']){
			...
			$UserinfoM=$this->MODEL('userinfo');
 			$userinfo = $UserinfoM->GetMemberOne(array("`moblie`='".$moblie."'"),array('field'=>"usertype,uid"));
			$binding = $UserinfoM->GetUserinfoOne(array("`uid`=".$userinfo['uid']." and `moblie_status`=1"),array('usertype'=>$userinfo['usertype'],'field'=>"uid"));
			if(!$binding){
				$this->layer_msg('手机号码未绑定!');
			}			
