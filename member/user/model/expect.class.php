...
class expect_controller extends user{
	...
	function index_action(){
		...
		if(!$_GET['e']){
			...
			if($num>=$this->config['user_number'] && $this->config['user_number']!=''){
				$this->ACT_msg("index.php?c=resume","你的简历数已经超过系统设置的简历数了".$num);
			}
		}
	...
	function saveexpect_action(){
		if($_POST['submit']){
			...
			if($eid){
				...
				$nid=$this->obj->update_once("resume_expect",$expectData,$where);
				...
				if($nid){
					...
					$resume=$this->obj->DB_select_once("resume_expect","`id`='".$eid."'","`id`,`name`,`job_classid`,`city_classid`,`hy`,`minsalary`,`maxsalary`,`report`,`type`,`jobstatus`");
					...
					if ($brforeresume){
					    ...
					    if($citynochange==1){
					        $this->obj->DB_delete_all('resume_cityclass', "`eid`='".$resume['id']."'","");
					        $this->morecity_insert($resume['id'], $_POST['city_classid'],$this->uid);
					    }
					    ...
					    if($jobnochange==1){
					        $this->obj->DB_delete_all('resume_jobclass', "`eid`='".$resume['id']."'","");
					        $this->morejob_insert($resume['id'], $_POST['job_classid'],$this->uid);
					    }
					}
					...
					if($resume['city_classid']!=""){
					    ...
					    $resume['city_classname']=implode(" ",$city_classname);
					}
	...
	function add_action(){
		...
		if($_POST['submit']){
			...
			elseif($_POST['maxsalary']>0 &&(int)$_POST['minsalary'] > (int)$_POST['maxsalary']){$this->ACT_layer_msg("最高薪资必须大于最低薪资！",8);}
			if($_POST['city_class']==""){$this->ACT_layer_msg("请选择工作地区！",8);}
        ...
        $_POST['maxsalary']=0;
			...
			if ($nid){
			    ...
			    $this->morecity_insert($nid,$_POST['city_class'],$this->uid);
			    $this->morejob_insert($nid,$_POST['job_class'],$this->uid);
