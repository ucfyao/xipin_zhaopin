...
class admin_resume_controller extends adminCommon{
		...
		if(is_array($rows)){
			foreach($rows as $k=>$v){
				...
				if(is_array($city_classid)){
				    ...
				    $rows[$k]['citynum']=$i;
				    $rows[$k]['cityid_n']=$city_name[$cityname[0]];
				}
				if($v['topdate']>$time){
	...
	function del_member($id){
		...
		if($id_arr[0]){
			...
			$del_array=array("resume_cert","resume_edu","resume_other","resume_project","resume_skill","resume_training","resume_work","resume_doc","user_resume","resume_show","down_resume","userid_job","user_entrust_record","talent_pool"); 
		...
		if($_POST['submit']){
			...
			if($eid==""){
				...
				if ($nid){
					...
					$this->morecity_insert($nid, $_POST['city_classid'],$_POST['uid']);
					$this->morejob_insert($nid, $_POST['job_classid'],$_POST['uid']);
					$data['uid'] = $_POST['uid'];
					$data['eid'] = $nid;
				...
				$brforeresume = $this->obj->DB_select_once("resume_expect","`id`='".$eid."'","`job_classid`,`city_classid`");
				...
				if($nid){
					if ($brforeresume){
						...
						if($citynochange==1){
							...
							$this->morecity_insert($eid,$_POST['city_classid'],$_POST['uid']);
						}
						...
						if($jobnochange==1){
							...
							$this->morejob_insert($eid, $_POST['job_classid'],$_POST['uid']);
						}
					}
			...
			if(!empty($list))
			{
				...
				foreach($list as $k=>$v)
				{
					...
					if($v['city_classid']!="")
					{
						...
						$list[$k]['city_classid']=@implode(",",$citys);
					}
				}
	...
	function citytransform_action(){
	    $resume = $this->obj->DB_select_all('resume_expect','1 order by `id` asc limit 0,10000','`id`,`city_classid`,`provinceid`,`cityid`,`three_cityid`');
	    if ($resume){
	        $resumecity=array();
	        foreach ($resume as $k=>$v){
	            if($v['city_classid']==''){
	                ...
	                }elseif ($v['three_cityid']<1 && $v['cityid']>0){
	                    $cityclass = $v['cityid'];
	                }else {
	                    $cityclass = $v['provinceid'];
	                }
	            }
	            $resumecity[$k]['id']=$v['id'];
	            $resumecity[$k]['city_classid']=$cityclass;
	        }
	        if ($resumecity){
	            $set = '`city_classid`=case id';
	            foreach ($resumecity as $v){
	                $sql.= ' when '.$v['id'].' then '.$v['city_classid'].'';
	                $ids[]=$v['id'];
	            }
	            $sql .=' end';
	            $where = 'id in ('.@pylode(',', $ids).')';
	            $nid = $this->obj->DB_update_all('resume_expect', $set.$sql, $where);
	        }
	    }
	...
	function jobtransform_action(){
	    $resume = $this->obj->DB_select_all('resume_expect',"`job_classid`<>'' order by `id` asc limit 101000,30000",'`id`,`job_classid`');
	    if ($resume){
	        include PLUS_PATH."job.cache.php";
	        foreach ($resume as $v){
	            $jobclass = @explode(',', $v['job_classid']);
	            foreach ($jobclass as $val){
	                $morejob=array();
	                $morejob['eid']=$v['id'];
	                $morejob['job1']=0;
	                $morejob['job1_son']=0;
	                $morejob['job_post']=0;
	                if (in_array($val, $job_index)){
	                    $morejob['job1']=$val;
	                }elseif (in_array($val, $job_two)){
	                    $morejob['job1_son']=$val;
	                    foreach ($job_type as $k2=>$v2){
	                        if (in_array($val, $v2)){
	                            $morejob['job1']=$k2;
	                        }
	                    }
	                }elseif (in_array($val, $job_three)){
	                    $morejob['job_post']=$val;
	                    foreach ($job_type as $k32=>$v32){
	                        if (in_array($val, $v32)){
	                            $morejob['job1_son']=$k32;
	                            foreach ($job_type as $k3=>$v3){
	                                if (in_array($k32, $v3)){
	                                    $morejob['job1']=$k3;
	                                }
	                            }
	        ...
	        if ($resume_job){
	            ...
	            $newkey=array('eid','job1','job1_son','job_post');
	            include CONFIG_PATH."db.config.php";
	            $sql = "INSERT INTO `".$db_config['def']."resume_jobclass` (`".@implode('`,`',$newkey)."`) VALUES ".@implode(',',$newval);
	            $this->obj->DB_query_all($sql);
	...
	function cityclasstransform_action(){
	    $resume = $this->obj->DB_select_all('resume_expect',"`city_classid`<>'' order by `id` asc limit 0,26000",'`id`,`uid`,`city_classid`');
	    if ($resume){
	        include PLUS_PATH."city.cache.php";
	        foreach ($resume as $v){
	            $cityclass = @explode(',', $v['city_classid']);
	            foreach ($cityclass as $val){
	                $morecity=array();
	                $morecity['eid']=$v['id'];
	                $morecity['uid']=$v['uid'];
	                $morecity['provinceid']=0;
	                $morecity['cityid']=0;
	                $morecity['three_cityid']=0;
	                if (in_array($val, $city_index)){
	                    $morecity['provinceid']=$val;
	                }elseif (in_array($val, $city_two)){
	                    $morecity['cityid']=$val;
	                    foreach ($city_type as $k2=>$v2){
	                        if (in_array($val, $v2)){
	                            $morecity['provinceid']=$k2;
	                        }
	                    }
	                }elseif (in_array($val, $city_three)){
	                    $morecity['three_cityid']=$val;
	                    foreach ($city_type as $k32=>$v32){
	                        if (in_array($val, $v32)){
	                            $morecity['cityid']=$k32;
	                            foreach ($city_type as $k3=>$v3){
	                                if (in_array($k32, $v3)){
	                                    $morecity['provinceid']=$k3;
	                                }
	                            }
	                        }
	                    }
	                }
	                $resume_city[]=$morecity;
	            }
	        }
	        if ($resume_city){
	            $list = array();
	            ...
	            include CONFIG_PATH."db.config.php";
	            $sql = "INSERT INTO `".$db_config['def']."resume_cityclass` (`".@implode('`,`',$newkey)."`) VALUES ".@implode(',',$newval);
	            $this->obj->DB_query_all($sql);
