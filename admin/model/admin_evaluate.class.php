...
class admin_evaluate_controller extends adminCommon{
	...
	function record_action(){
		...
		if(trim($_GET['keyword'])!=""){
			...
			if($_GET['type']=='1' || $_GET['type']==''){
				...
				$exameInfo = $this->obj->DB_select_all('evaluate_group',"`name` like '%".trim($_GET['keyword'])."%'",'id');
				if($exameInfo&&is_array($exameInfo)){
					foreach($exameInfo as $v){
						$examids[]=$v['id'];
					}
				}
				$where .=" and `examid` in (".pylode(',',$examids).")";
			}
			$urlarr['type']=$_GET['type'];
			$urlarr['keyword']=$_GET['keyword'];
		}
		...
		$urlarr['page']="{{page}}";
		$pageurl=Url($_GET['m'],$urlarr,'admin'); 
		$rows=$this->get_page("evaluate_log",$where,$pageurl,$this->config['sy_listnum'],"`id`,`uid`,`nuid`,`examid`,`usedsecond`,`grade`,`ctime`"); 
		...
		if($rows&&is_array($rows)){
			...
			$exame = $this->obj->DB_select_all("evaluate_group","`keyid`!='0'","`id`,`keyid`,`name`");
			
			foreach($rows as $key=>$val){
				foreach($exame as $v){
					if($val['examid'] == $v['id']){
						$rows[$key]['title'] = $v['name'];
						$rows[$key]['keyid'] = $v['keyid'];
					}
				}
			}
		} 
	...
	function delevaluatelog_action(){
		if(is_array($_GET['del'])){
			$layer_type=1;
			$nidd=$this->obj->DB_delete_all("evaluate_log","`id` in (".pylode(',',$_GET['del']).")"," ");
		}else if($_GET['id']){
			$layer_type=0;
			$nidd=$this->obj->DB_delete_all("evaluate_log","`id`='".intval($_GET['id'])."'");
		}
		if($nidd){
			$this->layer_msg( "测评记录删除成功！",9,$layer_type,$_SERVER['HTTP_REFERER']);
		}else{
			$this->layer_msg( "测评记录删除失败！",8,$layer_type,$_SERVER['HTTP_REFERER']);
		}
	}
