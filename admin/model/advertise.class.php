...
class advertise_controller extends adminCommon{
	...
	function ad_saveadd_action(){
    	...
    	if ($_POST['ad_type']=="pic"){
			$pictures = str_replace($this->config['sy_weburl']."/data", "../data", $_POST['ad_url']);
		}
	...
	function addclass_action(){
		if($_POST['class_name']){
			$data="`class_name`='".$_POST['class_name']."',`orders`='".$_POST['orders']."'";
			if($_POST['id']){
				$nid=$this->obj->DB_update_all("ad_class",$data,"`id`='".$_POST['id']."'");
				$msg=$nid?1:2;
				if($msg==1){
				...
				$nid=$this->obj->DB_insert_once("ad_class",$data);
				$msg=$nid?1:2;
				if($msg==1){
		...
		if($_GET['id']){
			$info=$this->obj->DB_select_once("ad_class","`id`='".intval($_GET['id'])."'");
			$this->yunset('info',$info);
		}
	...
	function delclass_action(){
		...
		if($_GET['del']){
	    	...
	    	if($del){
				if(is_array($del)){
					$ad = $this->obj->DB_select_once("ad","`class_id` in(".@implode(',',$del).")");
					if(is_array($ad)){
