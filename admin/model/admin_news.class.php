...
class admin_news_controller extends adminCommon{
	...
	function news_action(){
		...
		if($_GET['id']){
			...
			$info['content']=str_replace(array("&amp;","&nbsp;"),array("&",''),$row['content']);
		...
		if($keyname){
			...
			foreach($keyname as $val){
				if(preg_match("/^[\x{4e00}-\x{9fa5}]+$/u",$val['key_name'])){
					...
					$href[]='<a href="'.searchListRewrite(array('type'=>'keyword','v'=>$val['key_name']),$this->config).'"target="_bank"><span style="color:#E53333;">'.$val['key_name'].'</span></a>';
